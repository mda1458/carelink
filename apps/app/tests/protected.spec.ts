import { test, expect } from "@playwright/test"; import { SignJWT } from "jose";
const enc = new TextEncoder(); const secret = process.env.ROLE_COOKIE_SECRET || "testsecret";
async function signRole(role: string, sub?: string) { const now = Math.floor(Date.now()/1000);
...
test("authorized loads dashboard", async ({ page, context }) => { const token = await signRole("employer","u_1");
