...
import { isMimeAllowed, isSizeAllowed, sanitizeName } from "../../../src/lib/upload";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!isMimeAllowed(mime)) return json({ ok: false, error: `MIME not allowed: ${mime}` }, 415);
    ...
    return json({ ok: true, filename, mime, size: file.size }, 200);
  } catch (error) {
